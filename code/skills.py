    ...
    try:
        ...
        if start >= 0 and end > start:
            ...
            try:
                ...
                # The LLM often emits literal newlines inside JSON string
                # values (especially code). Escape them and retry.
                import re
                ...
                try:
                    ...
                    pass
                # Last resort: extract "code" field via regex for coder skill
                code_match = re.search(r'"code"\s*:\s*"(.*?)"(?:\s*,\s*"rationale")', blob, re.DOTALL)
                if code_match:
                    code_raw = code_match.group(1)
                    # Properly escape for JSON
                    code_escaped = code_raw.replace('\\', '\\\\').replace('\n', '\\n').replace('\r', '\\r').replace('\t', '\\t').replace('"', '\\"')
                    rationale_match = re.search(r'"rationale"\s*:\s*"([^"]*)"', blob)
                    rationale = rationale_match.group(1) if rationale_match else ""
                    return {"code": code_raw, "rationale": rationale}
    return {}
