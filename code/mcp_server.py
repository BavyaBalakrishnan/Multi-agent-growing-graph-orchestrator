...
@mcp.tool()
def check_fields(json_text: str, required_fields: list[str]) -> dict:
    """Check that a JSON object contains all required fields with non-empty values. Returns pass/fail with details. Example: check_fields('{"name":"Ada","age":36}', ["name","age","city"])."""
    try:
        obj = json.loads(json_text)
    except (json.JSONDecodeError, TypeError):
        return {"pass": False, "error": "invalid JSON", "missing": required_fields, "empty": []}
    if not isinstance(obj, dict):
        return {"pass": False, "error": "not a JSON object", "missing": required_fields, "empty": []}
    missing = [f for f in required_fields if f not in obj]
    empty = [f for f in required_fields if f in obj and not str(obj[f]).strip()]
    return {
        "pass": len(missing) == 0 and len(empty) == 0,
        "missing": missing,
        "empty": empty,
        "present": [f for f in required_fields if f in obj and str(obj[f]).strip()],
    }


@mcp.tool()
def check_exact(json_text: str, expected: dict[str, str]) -> dict:
    """Check that specific fields in a JSON object match expected values exactly. Returns pass/fail with mismatches. Example: check_exact('{"city":"Paris","year":"1889"}', {"city": "Paris", "year": "1889"})."""
    try:
        obj = json.loads(json_text)
    except (json.JSONDecodeError, TypeError):
        return {"pass": False, "error": "invalid JSON", "mismatches": expected}
    if not isinstance(obj, dict):
        return {"pass": False, "error": "not a JSON object", "mismatches": expected}
    mismatches = {}
    for field, expected_val in expected.items():
        actual = str(obj.get(field, "")).strip()
        if actual != expected_val.strip():
            mismatches[field] = {"expected": expected_val, "actual": actual or "(missing)"}
    return {
        "pass": len(mismatches) == 0,
        "mismatches": mismatches,
        "matched": [f for f in expected if f not in mismatches],
    }
